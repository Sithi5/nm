...
static bool is_valid_elf_magic(t_nm *nm) {
...
static bool is_valid_elf_class(t_nm *nm) {
...
static bool is_valid_os_abi(t_nm *nm) {
...
static bool is_valid_ei_data(t_nm *nm) {
...
static bool is_valid_version(t_nm *nm) {
    if (nm->mapped_data[EI_VERSION] != EV_CURRENT) {
        ft_dprintf(STDERR_FILENO, "%s: %s: Unsupported ELF file version\n", PROGRAM_NAME,
                   nm->current_filename);
        return false;
    }
    return true;
...
static bool wrong_elf_header(t_nm *nm) {
    close(nm->fd);
    munmap(nm->mapped_data, nm->mapped_data_info.st_size);
    return false;
}
